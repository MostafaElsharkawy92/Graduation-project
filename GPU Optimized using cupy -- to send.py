...
# Helper function to convert NumPy array to CuPy array
def np_to_cp(image):
    return cp.array(image)

# Helper function to convert CuPy array to NumPy array
def cp_to_np(image_cp):
    return cp.asnumpy(image_cp)

# CUDA-accelerated color selection (HSL)
...
def HSL_color_selection(image):
    ...
    white_mask = cp.in_range(np_to_cp(converted_image), lower_threshold, upper_threshold)

    # Yellow color mask
    lower_threshold = cp.uint8([10, 0, 100])
    upper_threshold = cp.uint8([40, 255, 255])
    yellow_mask = cp.in_range(np_to_cp(converted_image), lower_threshold, upper_threshold)

    # Combine white and yellow masks
    mask = cp.bitwise_or(white_mask, yellow_mask)
    masked_image = cp.bitwise_and(np_to_cp(image), np_to_cp(image), mask=mask)

    return cp_to_np(masked_image)
...
# CUDA-accelerated region of interest
def region_selection(image):
    mask = cp.zeros_like(np_to_cp(image))
    ...
    cp.fillPoly(mask, vertices, ignore_mask_color)
    masked_image = cp.bitwise_and(np_to_cp(image), mask)
