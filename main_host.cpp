...
int main(int argc, char* argv[]) {
	...
	unsigned int DATA_SIZE = smoothedIm.rows * smoothedIm.cols;
	...
	krnl.setArg(narg++, smoothedIm.rows);
	krnl.setArg(narg++, smoothedIm.cols);
	krnl.setArg(narg++, minVal);
	krnl.setArg(narg++, maxVal);
	...
	cv::Mat maskedIm = dst.clone();
	cv::bitwise_and(dst, mask, maskedIm);
