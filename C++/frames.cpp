...
int main(int argc, char** argv) {
	...
	cv::VideoCapture inputVideo("C:/Users/anwrk/source/repos/lane_detection_test2/lane_detection_test2/drivingVideo.mp4");

	if (!inputVideo.isOpened()) {
		std::cerr << "Error: Unable to open video file." << std::endl;
		return -1;
	}
	//cv::VideoWriter outputVideo( "dashcam5_out.mp4", cv::VideoWriter::fourcc('X', '2', '6', '4'));
	...
	while (inputVideo.read(frame)) {
		...
		//---------------GET frame---------------------
	 // Read the frame file
	 // TODO: Change to an argument, enter path to frame
	//cv::Mat frame = cv::imread("C:/Users/anwrk/source/repos/lane_detection_test2/lane_detection_test2/1.jpg"); // read the frame into mat type variable, frame, if funning from cmd line, path is ../src/1.jpg

	// Check for failure in reading the frame
	/*if (frame.empty()) // check if it's empty
	{
		cout << "Could not open or find the frame" << endl;
		return -1;
	}*/

	// name of window to show original frame
		cv::String originalWindowName = "Original frame";

		// Show the original frame
		//imshow(originalWindowName, frame);
		...
		// window for grayscaled frame
		cv::String grayscaleWindowName = "Grayscaled frame";

		// Show grayscale frame
		// cv::imshow(grayscaleWindowName, frameGray);
		...
		// Define smoothed frame
		cv::Mat smoothedIm;
		cv::GaussianBlur(frameGray, smoothedIm, cv::Size(kernelSize, kernelSize), 0, 0);

		// window for smoothed frame
		cv::String smoothedWindowName = "Smoothed frame";
		...
		// Define edge detection frame, do edge detection
		cv::Mat edgesIm;
		cv::Canny(smoothedIm, edgesIm, minVal, maxVal);

		// window for edge detection frame
		cv::String edgeWindowName = "edge detection frame";

		// Show edge detection frame
		...
		// Define masked frame
		// Create all black frame with same dimensions as original frame
		// 3rd arg is CV_<bit-depth>{U|S|F}C(<number_of_channels>), so this is 8bit, unsigned int, channels: 1
		cv::Mat mask(frame.size().height, frame.size().width, CV_8UC1, cv::Scalar(0)); // CV_8UC3 to make it a 3 channel
		...
		//---------------------APPLY MASK TO frame----------------------
		// create frame only where mask and edge Detection frame are the same
		...
		// Show masked frame
		// cv::imshow("Masked frame", maskedIm);
		// cv::waitKey(0)
		...
		if (!lines.empty() && lines.size() > 2) {
		   ...
		   // cout << endl;
			...
			// cv::imshow("Lane lines on frame", laneLineframe);
			// cv::waitKey(0) // wait for a key press
		...
		//	cv::imwrite("C:/Users/anwrk/source/repos/lane_detection_test2/lane_detection_test2/collected_frames/out.jpg", blendedIm);
