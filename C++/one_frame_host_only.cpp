...
//#include <xfopencv/include/imgproc/xf_canny.hpp>// Include XF OpenCV Canny header

#define HEIGHT 480 // Define image height
#define WIDTH 640  // Define image width
...
int main(int argc, char** argv) {
	...
	//---------------GET IMAGE---------------------
	 ...
	 //cv::Mat image = cv::imread("C:/Users/anwrk/source/repos/lane_detection_test2/lane_detection_test2/1.jpg"); // read the image into mat type variable, image, if funning from cmd line, path is ../src/1.jpg
	 cv::Mat image;
	 image = cv::imread("r_test_image.jpeg");
	 // Check for failure in reading the image
	 if (!image.data) // check if it's empty
	 {
	...
	// Plot positive and negative lane lines
	 ...
	 cv::imwrite("NC_lane_out.jpeg", edgesIm);
	 cout << "Execution time is "<< cpu_time_used<< endl;
     cout << "Created out_put image" << endl;
