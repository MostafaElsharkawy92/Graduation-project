...
int main(int argc, char* argv[]) {
	...
	int status = 0;
		...
		double kernal_time;
        clock_t start1, end1;
		double only_kernal_time;


	//--------------------------GET IMAGE---------------------------------------------------

	cv::Mat image;
	image = cv::imread("Test_img.jpeg");
	...
	if (!lines.empty() && lines.size() > 2) {
	   ...
	   // cout << endl;
		...
		/*unsigned char* ptr_in_1 = (unsigned char*)q.enqueueMapBuffer(buffer_in_1, CL_TRUE, CL_MAP_WRITE, 0, size_in_bytes);
		unsigned char* ptr_in_2 = (unsigned char*)q.enqueueMapBuffer(buffer_in_2, CL_TRUE, CL_MAP_WRITE, 0, size_in_bytes);
		...
		cv::imwrite("New_lane_out.jpg", blendedIm);
		kernal_time = ((double)(end - start)) / CLOCKS_PER_SEC;
	    cout << "kernal_time is  : " << kernal_time << endl;
        only_kernal_time = ((double)(end1 - start1)) / CLOCKS_PER_SEC;
	    cout << "only_kernal_time is  : " << only_kernal_time << endl;
        
		cout << "Created output image" << endl;
